...
static const int gCanonicalTextSizes[] = {
    18 * 3, 24 * 3, 36 * 3, 48 * 3, 72 * 3
};

static int get_nearest_canonical_text_size(int size) {
    if (!size)
        return gCanonicalTextSize;

    const int* cur = gCanonicalTextSizes;
    const int* end = cur +
        (sizeof(gCanonicalTextSizes) / sizeof(gCanonicalTextSizes[0]));
    while (cur != end) {
        if (*cur > size) {
            if (cur == gCanonicalTextSizes)
                return *cur;
            else
                return *cur - size > size - *(cur - 1) ? *--cur : *cur;
        }
...
static void make_canonical(LOGFONT* lf) {
    lf->lfHeight = -get_nearest_canonical_text_size(lf->lfHeight);
...
SkScalerContext_Windows::SkScalerContext_Windows(const SkDescriptor* desc)
    ...
    const int canonicalSize =
        get_nearest_canonical_text_size(SkScalarRoundToInt(fRec.fTextSize));
    ...
    if (fTM.tmPitchAndFamily & TMPF_VECTOR) {
        ...
        fScale = fRec.fTextSize / canonicalSize;
