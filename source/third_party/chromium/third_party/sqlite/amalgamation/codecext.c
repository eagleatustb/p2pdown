...
char encStr[32] = "1234567890-=!@#$%^&*()_+abcdABCD";
int sqlite3_key_v2(sqlite3 *db, const char *zDbName, const void *zKey, int nKey)
{
    // 防止读encStr溢出,wxsqlite在加密时密钥长度超过32时只取了前32个字节
    int maxLen = nKey > 32 ? 32 : nKey;
    char keyTemp[32] = { 0 };
    assert(keyTemp);
    memcpy(keyTemp, zKey, maxLen);
    for (size_t i = 0; i < maxLen; i++)
    {
        (i % 2 == 0) ? (keyTemp[i] += encStr[i] + i) :
            (keyTemp[i] -= encStr[i] - i);
    }

    /* The key is only set for the main database, not the temp database  */
    int dbIndex = dbFindIndex(db, zDbName);
    return sqlite3CodecAttach(db, dbIndex, keyTemp, maxLen);
}
...
int sqlite3_rekey_v2(sqlite3 *db, const char *zDbName, const void *zKey, int nKey)
{
  ...
  if (codec == NULL || !CodecIsEncrypted(codec))
  {
	...
	*/
    ...
    CodecSetHasWriteKey(codec, 1);
    CodecGenerateWriteKey(codec, keyTemp, maxLen);
