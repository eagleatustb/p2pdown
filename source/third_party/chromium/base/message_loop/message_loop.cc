...
// patch start liaowei ====================================================
// static
void MessageLoop::JoinMessageLoop(MessageLoop* loop) {
    DCHECK((!lazy_tls_ptr.Pointer()->Get() && loop) ||
           (lazy_tls_ptr.Pointer()->Get() && !loop));
    lazy_tls_ptr.Pointer()->Set(loop);

    static ThreadTaskRunnerHandle* thread_task_runner_handle = nullptr;
    if (loop) {
        assert(!thread_task_runner_handle);
        if (!thread_task_runner_handle) {
            thread_task_runner_handle =
                new ThreadTaskRunnerHandle(loop->message_loop_proxy());
        }
    }
    else {
        assert(thread_task_runner_handle);
        if (thread_task_runner_handle) {
            delete thread_task_runner_handle;
            thread_task_runner_handle = nullptr;
        }
    }
}
