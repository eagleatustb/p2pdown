...
class BASE_EXPORT SequencedWorkerPool : public TaskRunner {
 public:
  ...
  //
  // To ensure all tasks created by a specific object will not be run after a
  // certain point(usually a destructor call), as these tasks may probably
  // contain the object's references.
  //
  // When finalizing a token:
  //
  // 1. The call will hold until all the tasks associated with the token have
  //    been cleared/run, as per |behavior| specified in the task.
  // 2. The task associated with the token has already begun executing will
  //    continue running.
  //
  // NOTE: To use this feature, do *NOT* reuse any token that you are going to
  //       finalize.
