...
bool LaunchProcess(const string16& cmdline,
  ...
  if (options.as_user) {
    ...
    // patch start linkangming ==================================================
    // 由于这个分支暂时没用,使用CreateEnvironmentBlock和DestroyEnvironmentBlock
    // 会多引入一个dll,为了节省资源和加快启动速度,这里改为使用动态加载的
    typedef BOOL(__stdcall* CreateEnvironmentBlockType)(LPVOID, HANDLE, BOOL);
    typedef BOOL(__stdcall* DestroyEnvironmentBlockType)(LPVOID);
    HMODULE dll = LoadLibrary(L"userenv.dll");
    if (!dll)
        return false;

    CreateEnvironmentBlockType CreateEnvironmentBlockPtr =
        reinterpret_cast<CreateEnvironmentBlockType>(
        GetProcAddress(dll, "CreateEnvironmentBlock"));
    DestroyEnvironmentBlockType DestroyEnvironmentBlockPtr =
        reinterpret_cast<DestroyEnvironmentBlockType>(
        GetProcAddress(dll, "DestroyEnvironmentBlock"));

    if (!CreateEnvironmentBlockPtr || !DestroyEnvironmentBlockPtr) {
        FreeLibrary(dll);
        return false;
    }

    if (!CreateEnvironmentBlockPtr(&enviroment_block, options.as_user, FALSE)) {
      DPLOG(ERROR);
      return false;
    }
    ...
    BOOL launched =
        CreateProcessAsUser(options.as_user, NULL,
                            ...
                            &temp_process_info);
    DestroyEnvironmentBlockPtr(enviroment_block);
    FreeLibrary(dll);
    // patch end linkangming ==================================================
