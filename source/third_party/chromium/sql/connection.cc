...
bool Connection::IsDbEncrypted(const std::wstring& dbFilePath)
{
    base::File readFile(base::FilePath(dbFilePath),
    ...
    char sqliteFixedHead[] = "SQLite format 3";
    std::unique_ptr<char> fileHead(new char[16]);
    readFile.Read(0, fileHead.get(), 16);
    if (!strcmp(sqliteFixedHead, fileHead.get()))
    {
        return false;
    }
...
bool Connection::OpenWithKey(const base::FilePath& path, const std::wstring& key)
{
#if defined(OS_WIN)
    return OpenInternalWithKey(WideToUTF8(path.value()), WideToUTF8(key),
        base::PathExists(path));
#elif defined(OS_POSIX)
    return OpenInternalWithKey(path.value(), key, base::PathExists(path));
#endif
}

bool Connection::DbEncrypt(const std::wstring& key)
{
    if (!db_) {
        NOTREACHED() << "sql::Connection is not open.";
        return false;
    }

    std::string keyUtf8 = WideToUTF8(key).c_str();
    int err = sqlite3_rekey(db_, keyUtf8.c_str(), keyUtf8.size());
    if (err != SQLITE_OK)
    {
        NOTREACHED() << "sql::Connection clear encrypt failed.";
        Close();
        return false;
    }
...
bool Connection::ClearDbEncrypt()
{
    if (!db_) {
        NOTREACHED() << "sql::Connection is not open.";
        return false;
    ...
    int err = sqlite3_rekey(db_, nullptr, 0);
    if (err != SQLITE_OK)
    {
        NOTREACHED() << "sql::Connection clear encrypt failed.";
        Close();
...
bool Connection::OpenInternalWithKey(const std::string& file_name,
    const std::string& key, bool IsFileExist)
{
    if (db_) {
        NOTREACHED() << "sql::Connection is already open.";
        return false;
    ...
    if (!IsFileExist)
    {
        if (!DbEncrypt(base::UTF8ToWide(key)))
            return false;
    }
    else
    {
        // 解密
        err = sqlite3_key(db_, key.c_str(), key.size());
        if (err != SQLITE_OK)
        {
            Close();
            return false;
