...
class Connection {
 ...
 public:
     ...
     // 判断数据库是否加密
     static bool IsDbEncrypted(const std::wstring& dbFilePath);
  ...
  // 打开加密数据库,文件不存在时新建加密的数据库
  bool OpenWithKey(const base::FilePath& path, const std::wstring& key);

  // 数据库加密，调用前确保Open已被成功调用
  bool DbEncrypt(const std::wstring& key);

  // 清除数据库加密，调用前确保OpenWithKey已被成功调用
  bool ClearDbEncrypt();
 ...
 private:
  ...
  bool OpenInternalWithKey(const std::string& file_name, const std::string& key,
                           bool IsFileExist );
