...
PLT_MediaObject::FromDidl(NPT_XmlElementNode* entry)
{
	...
	res = PLT_XmlHelper::GetChildText(entry, "hash", m_Hash, didl_namespace_kg);
	if (NPT_FAILED(res))
	{
		m_Hash = "";
	}

	res = PLT_XmlHelper::GetChildText(entry, "username", m_UserName, didl_namespace_kg);
	if (NPT_FAILED(res))
	{
		m_UserName = "";
	}

	res = PLT_XmlHelper::GetChildText(entry, "phonename", m_DeviceName, didl_namespace_kg);
	if (NPT_FAILED(res))
	{
		m_DeviceName = "";
	}
    ...
    res = PLT_XmlHelper::GetChildText(entry, "current", m_StartDuration, didl_namespace_kg);
    if (NPT_FAILED(res))
    {
        m_StartDuration = "";
    }
