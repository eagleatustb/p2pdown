...
void File__Analyze::Open_Buffer_Continue (const int8u* ToAdd, size_t ToAdd_Size)
{
	...
	// 补丁之中的补丁!!!
	//
	// 这里解决某些文件结尾有垃圾数据导致死循环的问题
	//
	// 原因好像是MediaInfo分析到MP3帧信息之后会跳过后面的帧
	// 然后开始检测文件结尾的TAG信息，但在检测ID3v1的时候
	// 认为前面跳过的信息之后会在FileSize-128之后
	// 如果小于FileSize-128会一直调当前位置到FileSize-128-32
	// 而某些文件结尾有垃圾数据导致死循环
	const unsigned short MAX_LOOP_TIMES = 250;
	unsigned short wTimes(0);
